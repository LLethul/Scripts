...
local window = lib:MakeWindow({Name = "Deez"})
local main = window:MakeTab({Name = "Main"})

main:AddToggle({Name = "Silent Aim", Default = false, Callback = function(v)
    _G.Aimbot = v
end})
main:AddToggle({Name = "Reach", Default = false, Callback = function(v)
    _G.Reach = v
end})
main:AddToggle({Name = "Guard", default = false, Callback = function(v)
    if v then
...
main:AddSlider({Name = "WalkSpeed Toggle", Min = 16, Max = 19, Default = 16, Color = Color3.fromRGB(80, 80, 255), Increment = 0.1, Callback = function(v)
    _G.WS = v
end})

main:AddSlider({Name = "Reach Range", Min = 8, Max = 20, Default = 8, Color = Color3.fromRGB(255, 123, 0), Increment = 0.01, Callback = function(v)
    _G.RR = v
end})
    ...
    for i,v in pairs(game.Players:GetPlayers()) do
        if (v.Name ~= plr.Name and v.Character and plr.Character) and _G.Reach then
            ...
            for z,x in pairs(v.Character:GetChildren()) do
                if ((nearestPart.Position - v.Character.Torso.Position).Magnitude < (_G.RR == nil and 8 or _G.RR)) then
